...
parser = argparse.ArgumentParser(description='Create a Secret on the Nillion network; set with peer read permissions')
parser.add_argument('--reader_user_id', required=True, type=str, help='Peer ID of the reader python client (derived from user key)')
...
async def main():
    ...
    result = "allowed" if writer_permission.is_retrieve_allowed(args.reader_user_id) else "not allowed"
